...
$(function(){
	...
	var dir_name = [["소요산행","인천행"], ["내선: 시계방향", "외선: 반시계방향"],["대화행","오금행"], ["당고개행","남태령행"],["방화행", "하남검단산, 마천행"], ["응암행","신내행"],["장암행", "부평구청행"],["암사행", "모란행"], ["개화행", "중앙보훈병원행"]];
	...
	function drawCrevassePath(stationId, dir, count_accident){
		...
		if( dir=="up"){
			svgId = "CREVASSE_UP";
			keyStr = "entrances_up";
			if(stationId>=178 && stationId<=182){
				$("#"+svgId).siblings("h4").find("span.dir").html("승강장 없음");
			}else if(stationId==177){
				$("#"+svgId).siblings("h4").find("span.dir").html("역촌방향");
			}else{
			...
			if(stationId>=178 && stationId<=182){
				$("#"+svgId).siblings("h4").find("span.dir").html("응암행 순환선");
			}else if(stationId==177){
				$("#"+svgId).siblings("h4").find("span.dir").html("신내행");
			}else{
	...
	$(window).scroll(function(){
		...
		if(nowScroll+screenHeight*0.8 > $(".metro-select-info-border").offset().top && metroAni ==false){
			console.log("test");
			$("#path-start-deco").animate({"top":"0px", "opacity":"1"}, 1000, "easeOutBounce");
			metroAni = true;
		}
	...
	$(".metro-select-holder").on("mousedown", function(){
		$("#click-01").fadeOut();
	});
	$(".metro-select-holder").on("touchstart", function(){
		$("#click-01").fadeOut();
	});
